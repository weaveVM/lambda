...
use std::{any, fmt::format, fs::File, io::Split};
use structopt::StructOpt;
use ethers::{core::k256::elliptic_curve::rand_core::le, utils::hex};
...
pub fn encode_ethabi(input: &String) -> anyhow::Result<String> {
    {
        let ethabi_cmd : String = String::from("ethabi encode params -v string ");
        let command_content: String = format!("{}{}", ethabi_cmd, input);
        let command: std::str::Split<char> = command_content.split(' ');
        let res: String = execute(command).unwrap();
        Ok(res)
    }
}


pub fn decode_ethabi(input: &String) -> anyhow::Result<String> {
    {
        let ethabi_cmd : String = String::from("ethabi decode params -t string ");
        let command_content: String = format!("{}{}", ethabi_cmd, input);
        let command:std::str::Split<char> = command_content.split(' ');
        let res: String = execute(command).unwrap().replace("string ", "");
        Ok(res)
    }
}

pub fn execute<I>(args: I) -> anyhow::Result<String>
where
	I: IntoIterator,
